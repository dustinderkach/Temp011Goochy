...
interface Temp011GoochyAuthStackProps extends StackProps {
	envName: string;
	photosBucket: IBucket;
	env: { account: string; region: string };
}

export class Temp011GoochyAuthStack extends Stack {
	public userPool: UserPool;
	private props: Temp011GoochyAuthStackProps;
	...
	constructor(
		scope: Construct,
		id: string,
		props: Temp011GoochyAuthStackProps
	) {
	...
	private createUserPool() {
		this.userPool = new UserPool(
			this,
			`${this.props.envName}-Temp011GoochyUserPool`,
			{
		...
		new CfnOutput(this, `${this.props.envName}-Temp011GoochyUserPoolId`, {
	...
	private createUserPoolClient() {
		this.userPoolClient = this.userPool.addClient(
			`${this.props.envName}-Temp011GoochyUserPoolClient`,
			{
				authFlows: {
		...
		new CfnOutput(
			this,
			`${this.props.envName}-Temp011GoochyUserPoolClientId`,
			{
	...
	private createAdminsGroup() {
		new CfnUserPoolGroup(
			this,
			`${this.props.envName}-Temp011GoochyAdmins`,
			{
	...
	private createIdentityPool() {
		this.identityPool = new CfnIdentityPool(
			this,
			`${this.props.envName}-Temp011GoochyIdentityPool`,
			{
		...
		new CfnOutput(
			this,
			`${this.props.envName}-Temp011GoochyIdentityPoolId`,
			{
