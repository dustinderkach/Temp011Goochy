...
export default function CreateTemp011Goochy({ dataService }: CreateTemp011GoochyProps) {
	...
	const handleSubmit = async (event: SyntheticEvent) => {
		...
		if (name && location) {
			const id = await dataService.createTemp011Goochy(
				name,
				location,
				photo
			);
			setActionResult(`Created temp011Goochy with id ${id}`);
			setName("");
			setLocation("");
		} else {
	...
	function renderForm() {
		...
		return (
			<form onSubmit={(e) => handleSubmit(e)}>
				...
				{renderPhoto()}
				<br />
				<input type="submit" value="Create temp011Goochy" />
			</form>
		);
	}
